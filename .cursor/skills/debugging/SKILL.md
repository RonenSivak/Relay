...
Every debugging workflow follows the same loop:
...
1. **Observe**: Gather evidence (logs, errors, network, metrics)
2. **Hypothesize**: Form a theory about the cause
3. **Test**: Validate or invalidate the theory with targeted checks
4. **Verify**: Confirm the fix resolves the issue without side effects

**Rule**: Never skip straight to fixing. Always observe first.

**Tip**: Check Wix internal docs (`search_docs`, `fqdn_info`) early — it's a cheap, fast hypothesis source. Docs may surface known errors, expected behavior, or usage gotchas. Findings feed the **Hypothesize** step (not conclusions) and still need validation through logs/code. Docs may also return nothing useful — that's fine, move on.
...
```
1. Chrome DevTools: list_console_messages     → Check for JS errors
2. Chrome DevTools: list_network_requests     → Check for failed requests
3. Chrome DevTools: take_snapshot             → Inspect current DOM state
4. Chrome DevTools: take_screenshot           → Visual verification
5. Wix Docs: search_docs                     → Quick check for known issues / usage (may find nothing)
```
...
```
1. Wix Docs: search_docs / fqdn_info        → Quick check for known issues (may find nothing)
...
1. **Frontend**: `list_network_requests` → Find the failing request
2. **Get requestId**: From response headers (`x-request-id`) or `evaluate_script`
3. **Backend logs**: `query_loki_logs` with the requestId
4. **Root Cause**: `start_root_cause_analysis(requestId)` if complex
5. **Code trace**: Follow the request path through code with octocode LSP
