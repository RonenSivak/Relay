...
**MANDATORY**: Before executing wds-docs, ALWAYS invoke `/brainstorming` first to:

1. **Clarify what UI is needed** - What's the user trying to build?
2. **Identify design requirements** - Any specific layout, interactions, or constraints?
3. **Define success criteria** - What components would solve this?

Example flow:
```
User: "How do I build a product card?"

Step 1: /brainstorming
- Clarify: What goes in the card? (image, title, price, button?)
- Requirements: Clickable? Fixed width? Responsive?
- Success: Need Card + Image + Text + Button components

Step 2: /wds-docs
- Execute staged discovery for each identified component
```
