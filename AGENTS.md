...
| **e2e-testing** | E2E browser testing with Sled 2/3 or Playwright. Adaptive parallel subagents for detection, writing, and debugging. BDD driver pattern | [SKILL.md](.cursor/skills/e2e-testing/SKILL.md) |
...
| **implement-bi** | End-to-end BI event implementation for yoshi-flow-bm React projects. Adaptive parallel execution — groups events by component, dispatches subagents per group. plan.md + def-done.md verification. Orchestrates BI Catalog MCP and Eventor MCP | [SKILL.md](.claude/skills/implement-bi/SKILL.md) |
| **migrate-to-vitest** | Migrates test infrastructure from Jest to Vitest. Detects project type (serverless, yoshi-flow-bm, editor-flow, fed-cli, standalone), checks for Wix-specific blockers, generates plan with user approval, executes config + test file transforms. Backed by 8+ real Wix production repos. | [SKILL.md](.claude/skills/migrate-to-vitest/SKILL.md) |
